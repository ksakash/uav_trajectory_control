...
class DPControllerLocalPlanner(object):
    ...
    def has_reached_pose(self):
        dist = (self._curr_pose.position.x - self._end_pose.position.x)**2 + \
               (self._curr_pose.position.y - self._end_pose.position.y)**2 + \
               (self._curr_pose.position.z - self._end_pose.position.z)**2 + \
               (self._curr_pose.orientation.x - self._end_pose.orientation.x)**2 + \
               (self._curr_pose.orientation.y - self._end_pose.orientation.y)**2 + \
               (abs(self._curr_pose.orientation.z) - abs(self._end_pose.orientation.z))**2 + \
               (self._curr_pose.orientation.w - self._end_pose.orientation.w)**2
        return dist < 0.005
